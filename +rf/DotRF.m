...
classdef DotRF < dj.Computed
	...
	methods(Access=protected)
		...
		function makeTuples(self, key)
            ...
            key.gauss_fit = self.fitGauss(max(key.response_map,[],3), deg2dot, rf_filter);
            key.snr = self.rfSNR(key.gauss_fit, max(key.response_map,[],3),sd, snr_method);
        ...
        function SNR = rfSNR(self,par,z,sd, method) % compute RF SNR
            sz = size(z);
