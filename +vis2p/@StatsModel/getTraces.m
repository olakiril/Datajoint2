function Trace = getTraces( obj )

% fetch the traces that this object used
%
% organized in [cells classes trials]

keys = fetch(obj);

Trace = cell(length(keys),1);
...
% uncell if only a single key is given
if ikey == 1
    Trace = Trace{1};
end