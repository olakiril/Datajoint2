...
for k = 1:length(keys); key = keys(k);
    ...
    % get extra params
    [bin, rsThr, Qual, rpThr, eThr]= fetch1(StatsSitesParams(key),...
        'binsize','rf_snr_thr','trace_qual','rf_p_thr','event_thr');
...
%     if ~isnan(Qual); Tr = Traces(key,['quality>' num2str(Qual)]);
%     else Tr = Traces(key);
%     end
...
%     if ~isnan(rpThr);Tr=Tr.*RFStats(key,['onpoff_p<' num2str(rpThr)]);end
    ...
    times = trresize(times',fps,params.bin,'conv');
