...
for k = 1:length(keys)
    ...
    % get trace
    [traces masknum] = fetchn(Traces(key),'trace','masknum');
    traces{masknum == 0} = normalize(traces{masknum == 0})* 10;
    ...
    for iMov = 1:length(uniMovies);
        ...
        for iMovieType = 1:2
            ...
            traces = squeeze(mean(cell2mat(cellfun(@(x) permute(x,[3 1 2]),traces,'uniformoutput',0)'),1));
            ...
            plot(tt,bsxfun(@plus,traces, ...
                max(traces(:)):max(traces(:)):max(traces(:))*(size(traces,2))),...
