function [T, binsize] = getExtraTraces(obj,type,varargin)
...
params.raw = 0;      % spit out the filtered trace
...
for k = 1:length(keys)
    ...
    % get trace
    traces = fetchn(ExtraTraces(key),type);
    if isempty(traces);continue;end
    tracesM = cell2mat(traces');
    if strcmp(type,'eye_trace');
        ft = gausswin(round(fetch1(Movies(key),'fps')*2));
        tracesM = convn(tracesM,ft/sum(ft),'same');
    end
